...
(
	...
	if [ ! -L "/data/data/com.termux/files/usr/etc/termux/termux-bootstrap/second-stage/termux-bootstrap-second-stage.sh.lock" ]; then
		echo "Starting fallback run of termux bootstrap second stage"
		chmod +x "/data/data/com.termux/files/usr/etc/termux/termux-bootstrap/second-stage/termux-bootstrap-second-stage.sh" || exit $?
		"/data/data/com.termux/files/usr/etc/termux/termux-bootstrap/second-stage/termux-bootstrap-second-stage.sh" || exit $?
	fi

	# Delete script itself so that it is never run again
	rm -f "/data/data/com.termux/files/usr/etc/profile.d/01-termux-bootstrap-second-stage-fallback.sh" || exit $?
